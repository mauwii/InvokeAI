...
class WeightedPromptFragmentsToEmbeddingsConverter:
    ...
    def get_embeddings_for_weighted_prompt_fragments(
        ...
        if len(text) != len(fragment_weights):
            raise ValueError(
                f"lengths of text and fragment_weights lists are not the same ({len(text)} != {len(fragment_weights)})"
            )
        ...
        for fragments, weights in zip(text, fragment_weights):
            ...
            # handle weights >=1
            tokens, per_token_weights = self.get_token_ids_and_expand_weights(
                fragments, weights, device=device
            )
            base_embedding = self.build_weighted_embedding_tensor(
                tokens, per_token_weights
            )
            ...
            for index, fragment_weight in enumerate(weights):
                if fragment_weight < 1:
                    ...
                    tokens, per_token_weights = self.get_token_ids_and_expand_weights(
                        fragments_without_this, weights_without_this, device=device
                    )
    ...
    def get_token_ids_and_expand_weights(
        self, fragments: list[str], weights: list[float], device: str
    ...
    def build_weighted_embedding_tensor(
        self, token_ids: torch.Tensor, per_token_weights: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        batch_weights_expanded = (
            per_token_weights.reshape(per_token_weights.shape + (1,))
            .expand(z.shape)
            .to(z)
        )
