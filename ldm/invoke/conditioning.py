...
from ..modules.prompt_to_embeddings_converter import (
    WeightedPromptFragmentsToEmbeddingsConverter,
)
from .prompt_parser import (
    Blend,
    CrossAttentionControlledFragment,
...
def get_uc_and_c_and_ec(
    ...
    prompt, negative_prompt = get_prompt_structure(
        prompt_string, skip_normalize_legacy_blend=skip_normalize_legacy_blend
    )
    conditioning = _get_conditioning_for_prompt(
        prompt, negative_prompt, model, log_tokens
    )
...
def get_prompt_structure(
    ...
    """
    prompt, negative_prompt = _parse_prompt_string(
        prompt_string, skip_normalize_legacy_blend=skip_normalize_legacy_blend
    )
    return prompt, negative_prompt


def get_tokens_for_prompt(
    model, parsed_prompt: FlattenedPrompt, truncate_if_too_long=True
) -> [str]:
    text_fragments = [
        x.text
...
def _parse_prompt_string(
    prompt_string_uncleaned, skip_normalize_legacy_blend=False
) -> Union[FlattenedPrompt, Blend]:
    ...
    legacy_blend: Blend = pp.parse_legacy_blend(
        prompt_string_cleaned, skip_normalize_legacy_blend
    )
    ...
    parsed_negative_prompt: FlattenedPrompt = pp.parse_conjunction(
        unconditioned_words
    ).prompts[0]
    return parsed_prompt, parsed_negative_prompt


def _get_conditioning_for_prompt(
    parsed_prompt: Union[Blend, FlattenedPrompt],
    parsed_negative_prompt: FlattenedPrompt,
    model,
    log_tokens=False,
    ...
    if type(parsed_prompt) is Blend:
        ...
        if parsed_prompt.wants_cross_attention_control:
            conditioning, cac_args = _get_conditioning_for_cross_attention_control(
                model, parsed_prompt, log_tokens
            )

        else:
            conditioning, _ = _get_embeddings_and_tokens_for_prompt(
                model,
                parsed_prompt,
        ...
        raise ValueError(
            f"parsed_prompt is '{type(parsed_prompt)}' which is not a supported prompt type"
        )

    unconditioning, _ = _get_embeddings_and_tokens_for_prompt(
        model,
    ...
    if isinstance(conditioning, dict):
        ...
        unconditioning, conditioning = _flatten_hybrid_conditioning(
...
def _get_conditioning_for_cross_attention_control(
    model, prompt: FlattenedPrompt, log_tokens: bool = True
):
    ...
    original_token_count += 1
    edited_token_count += 1
    original_embeddings, original_tokens = _get_embeddings_and_tokens_for_prompt(
        model,
        original_prompt,
        log_tokens=log_tokens,
    ...
    edited_embeddings, edited_tokens = _get_embeddings_and_tokens_for_prompt(
        model,
        edited_prompt,
        log_tokens=log_tokens,
...
def _get_conditioning_for_blend(model, blend: Blend, log_tokens: bool = False):
    ...
    for i, flattened_prompt in enumerate(blend.prompts):
        this_embedding, _ = _get_embeddings_and_tokens_for_prompt(
            model,
            flattened_prompt,
            log_tokens=log_tokens,
        ...
        embeddings_to_blend = (
            this_embedding
            if embeddings_to_blend is None
            else torch.cat((embeddings_to_blend, this_embedding))
        )
    conditioning = WeightedPromptFragmentsToEmbeddingsConverter.apply_embedding_weights(
        embeddings_to_blend.unsqueeze(0),
        blend.weights,
...
def _get_embeddings_and_tokens_for_prompt(
    model,
    ...
    if type(flattened_prompt) is not FlattenedPrompt:
        raise Exception(
            f"embeddings can only be made from FlattenedPrompts, got {type(flattened_prompt)} instead"
        )
    fragments = [x.text for x in flattened_prompt.children]
    weights = [x.weight for x in flattened_prompt.children]
    embeddings, tokens = model.get_learned_conditioning(
        [fragments], return_tokens=True, fragment_weights=[weights]
    )
    if log_tokens or getattr(Globals, "log_tokenization", False):
        text = " ".join(fragments)
...
def _get_tokens_length(model, fragments: list[Fragment]):
    ...
    tokens = model.cond_stage_model.get_token_ids(
        fragment_texts, include_start_and_end_markers=False
    )
...
def _flatten_hybrid_conditioning(uncond, cond):
    ...
    for k in cond:
        if isinstance(cond[k], list):
            cond_flattened[k] = [
                torch.cat([uncond[k][i], cond[k][i]]) for i in range(len(cond[k]))
            ]
        else:
