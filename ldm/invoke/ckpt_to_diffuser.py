    ...
    with warnings.catch_warnings():
        ...
        if original_config_file is None:
            ...
            if key_name in checkpoint and checkpoint[key_name].shape[-1] == 1024:
                ...
                str(checkpoint_path).lower().find("inpaint") >= 0
            ):  # brittle - please pass original_config_file parameter!
                print(
                    f'  | checkpoint has "inpaint" in name, assuming an inpainting model'
                )
                original_config_file = (
                    global_config_dir()
                    / "stable-diffusion"
