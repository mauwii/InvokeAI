...
class Prompt:
    ...
    def __init__(self, parts: list):
        for c in parts:
            if (
                type(c) is not Attention
                and not issubclass(type(c), BaseFragment)
                and type(c) is not pp.ParseResults
            ):
                raise PromptParser.ParsingException(
                    f"Prompt cannot contain {type(c).__name__} ({c}), only {[c.__name__ for c in BaseFragment.__subclasses__()]} are allowed"
                )
        self.children = parts
...
class FlattenedPrompt:
    ...
    def append(self, fragment: Union[list, BaseFragment, tuple]):
        ...
        if type(fragment) is list:
            ...
            if type(fragment[0]) is not str or (
                type(fragment[1]) is not float and type(fragment[1]) is not int
            ):
                raise PromptParser.ParsingException(
...
class CrossAttentionControlAppend(CrossAttentionControlledFragment):
    ...
    def __eq__(self, other):
        return (
            type(other) is CrossAttentionControlAppend
            and other.fragment == self.fragment
        )
...
class Conjunction:
    ...
    def __init__(self, prompts: list, weights: list = None):
        ...
        self.prompts = [
            x
            if (type(x) is Prompt or type(x) is Blend or type(x) is FlattenedPrompt)
            else Prompt(x)
            for x in prompts
        ]
        self.weights = (
...
class Blend:
    ...
    def __init__(
        self, prompts: list, weights: list[float], normalize_weights: bool = True
    ):
        ...
        for p in prompts:
            ...
            for f in p.children:
                if isinstance(f, CrossAttentionControlSubstitute):
                    raise (
                        PromptParser.ParsingException(
                            f"while parsing Blend: sorry, you cannot do .swap() as part of a Blend"
                        )
                    )

        # upcast all lists to Prompt objects
        self.prompts = [
            x if (type(x) is Prompt or type(x) is FlattenedPrompt) else Prompt(x)
            for x in prompts
        ]
...
class PromptParser:
    ...
    def __init__(self, attention_plus_base=1.1, attention_minus_base=0.9):
        self.conjunction, self.prompt = build_parser_syntax(
            attention_plus_base, attention_minus_base
        )
    ...
    def parse_legacy_blend(
        self, text: str, skip_normalize: bool = False
    ) -> Optional[Blend]:
        weighted_subprompts = split_weighted_subprompts(
            text, skip_normalize=skip_normalize
        )
        ...
        return Blend(
            ...
            weights=weights,
            normalize_weights=not skip_normalize,
        )
                ...
                if type(x) is CrossAttentionControlSubstitute:
                    ...
                    result.append(
                        CrossAttentionControlSubstitute(
                            original_fused, edited_fused, options=x.options
                        )
                    )
                else:
                    last_weight = (
            ...
            if type(node) is pp.ParseResults or type(node) is list:
                for x in node:
                    results = flatten_internal(
                        x, weight_scale, results, prefix + " pr "
                    )
                ...
                for index, c in enumerate(node.children):
                    results = flatten_internal(
                        c, weight_scale * node.weight, results, prefix + f" att{index} "
                    )
                ...
                original = flatten_internal(
                    node.original, weight_scale, [], prefix + " CAo "
                )
                edited = flatten_internal(
                    node.edited, weight_scale, [], prefix + " CAe "
                )
                results += [
                    CrossAttentionControlSubstitute(
                        original, edited, options=node.options
                    )
                ]
                ...
                for prompt in node.prompts:
                    ...
                    flattened_subprompts = flatten_internal(
                        prompt, weight_scale, flattened_subprompts, prefix + "B "
                    )
                results += [
                    Blend(
                        prompts=flattened_subprompts,
                        weights=node.weights,
                        normalize_weights=node.normalize_weights,
                    )
                ]
...
def build_parser_syntax(attention_plus_base: float, attention_minus_base: float):
    def make_operator_object(x):
        ...
        if operator == ".attend":
            ...
            return Blend(
                prompts=prompts, weights=weights, normalize_weights=normalize_weights
            )
    ...
    number = pp.pyparsing_common.real | pp.Combine(
        pp.Optional("-") + pp.Word(pp.nums)
    ).set_parse_action(pp.token_map(float))
        ...
        pp.OneOrMore(
            pp.MatchFirst(
                [
                    ...
                    pp.one_of(["-", "+"])
                    + pp.NotAny(pp.White() | pp.Char(syntactic_chars) | pp.StringEnd()),
    ...
    free_word = pp.CharsNotIn(string.whitespace).set_parse_action(
        lambda x: Fragment(x[0])
    )
    ...
    fragment_including_commas = pp.ZeroOrMore(
        pp.MatchFirst(
            fragment_part_expressions
    ...
    quoted_fragment.set_parse_action(
        lambda x: parse_fragment_str(x, fragment_including_commas, in_quotes=True)
    )
    ...
    potential_operator_target = (
        quoted_fragment | parenthesized_fragment | non_syntax_word
    )
    ...
    attention_implicit.set_parse_action(
        lambda x: make_operator_object([x[0], ".attend", x[1]])
    )
    ...
    cross_attention_substitute << (
        ...
        + lparen
        + pp.Group(fragment_excluding_commas)
        .set_name("ca-replacement")
        .set_debug(False)
    ...
    quoted_prompt = quoted_fragment.copy().set_parse_action(
        lambda x: parse_fragment_str(x, prompt, in_quotes=True)
    )
    ...
    blend = (
        ...
        + pp.Group(
            pp.delimited_list(
                pp.Group(potential_operator_target | quoted_prompt), min=1
            )
        )
        .set_name("bl-target")
        .set_debug(False)
