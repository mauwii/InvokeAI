...
def import_checkpoint_list(models: List[Path], gen, opt, completer) -> List[str]:
    ...
    choice = (
        input(
            "** Directory of checkpoint/safetensors models detected. Install <a>ll or <s>elected models? [a] "
        )
        or "a"
    )
...
def import_diffuser_model(
    ...
    vae = None
    if click.confirm(
        'Replace this model\'s VAE with "stabilityai/sd-vae-ft-mse"?', default=False
    ):
...
def import_ckpt_model(
    ...
    base_name = (
        Path(url_attachment_name(path_or_url)).name
        if is_a_url
        else Path(path_or_url).name
    )
...
def _ask_for_config_file(
    model_path: Union[str, Path], completer, plural: bool = False
) -> Path:
    default = "1"
...
[4] Something else"""
        if plural
        else """What type of model is this?:
...
[4] Something else"""
    ...
    while not done:
        config_path = input(
            "Configuration file for this model (leave blank to abort): "
        ).strip()
...
def optimize_model(model_name_or_path: Union[Path, str], gen, opt, completer):
    ...
    if model_name_or_path == gen.model_name:
        ...
        original_config_file = original_config_file or _ask_for_config_file(
            model_name_or_path, completer
        )
